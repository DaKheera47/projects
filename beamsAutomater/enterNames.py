...
for i, student in enumerate(data, start=1):
    ...
    for idx, subject in enumerate(student["compulsory"], start=1):
        isFound = False
        if subject != "":
            for code in codes:
                if code["name"] == subject:
                    if idx != len(student["compulsory"]):
                        pag.write(f"{subject} ({code['code']}), ")
                        isFound = True
                    else:
                        pag.write(f"{subject} ({code['code']})")
                        isFound = True
    ...
    pag.press("tab")
    for idx, subject in enumerate(student["optional"], start=1):
        isFound = False
        if subject != "":
            for code in codes:
                if code["name"] == subject:
                    if idx != len(student["optional"]):
                        pag.write(f"{subject} ({code['code']}), ")
                        isFound = True
                    else:
                        pag.write(f"{subject} ({code['code']})")
